...
export class OpenGLManager {
  ...
  checkGLError(message: string) {
    ...
    if (error !== gl.NO_ERROR) {
      ...
      console.error(
        `OpenGL Error (${message}): ${error} - ${errorMessages[error] || "Unknown error"}`,
      );
  ...
  private compileShader(source: string, type: gl.GLenum): gl.GLuint | null { // Use appropriate return type if not WebGLShader
    ...
    if (status[0] === gl.FALSE) { // Check against gl.FALSE explicitly
      ...
      if (logLength[0] > 0) {
        ...
        console.error(
          `Shader Compile Error (type ${typeString}, ID ${shader}): No info log available, but compile status is FALSE.`,
        );
      }
  ...
  Version:  ${
        version
          ? new TextDecoder().decode(Deno.UnsafePointerView.getArrayBuffer(version, 20))
          : "N/A"
      }...
  Vendor:   ${
        vendor ? new TextDecoder().decode(Deno.UnsafePointerView.getArrayBuffer(vendor, 20)) : "N/A"
      }...
  Renderer: ${
        renderer
          ? new TextDecoder().decode(Deno.UnsafePointerView.getArrayBuffer(renderer, 20))
          : "N/A"
      }...`);
      ...
      this.uniformLocations.eyeLeft = gl.GetUniformLocation(
        this.shaderProgram!,
        cstr(eyeLeftLocName),
      );
      ...
      this.uniformLocations.eyeRight = gl.GetUniformLocation(
        this.shaderProgram!,
        cstr(eyeRightLocName),
      );
      ...
      this.uniformLocations.lookRotation = gl.GetUniformLocation(
        this.shaderProgram!,
        cstr(lookRotLocName),
      );
      ...
      this.uniformLocations.halfFOVInRadians = gl.GetUniformLocation(
        this.shaderProgram!,
        cstr(fovLocName),
      );
      ...
      try {
        ...
        if (!reprojectionLinkStatus[0]) {
          console.error(
            `!!! Reprojection shader program ID ${this.reprojectionShaderProgram} linking FAILED !!!`,
          );
          ...
          console.error(
            `--- Reprojection Program Link Log (ID ${this.reprojectionShaderProgram}) ---`,
          );
          console.error(new TextDecoder().decode(log.slice(0, logLength[0])));
        ...
        this.reprojectionUniformLocations.renderPoseMatrix = gl.GetUniformLocation(
          this.reprojectionShaderProgram,
          cstr("renderPose"),
        );
        this.reprojectionUniformLocations.currentPoseMatrix = gl.GetUniformLocation(
          this.reprojectionShaderProgram,
          cstr("currentPose"),
        );
        this.reprojectionUniformLocations.halfFOVInRadians = gl.GetUniformLocation(
          this.reprojectionShaderProgram,
          cstr("halfFOVInRadians"),
        ); // Still needed
        this.reprojectionUniformLocations.eyeLeft = gl.GetUniformLocation(
          this.reprojectionShaderProgram,
          cstr("eyeLeft"),
        );
        this.reprojectionUniformLocations.eyeRight = gl.GetUniformLocation(
          this.reprojectionShaderProgram,
          cstr("eyeRight"),
        );
        ...
        if (
          this.reprojectionUniformLocations.renderPoseMatrix === null ||
          this.reprojectionUniformLocations.renderPoseMatrix === -1 ||
          this.reprojectionUniformLocations.currentPoseMatrix === null ||
          this.reprojectionUniformLocations.currentPoseMatrix === -1 ||
          this.reprojectionUniformLocations.halfFOVInRadians === null ||
          this.reprojectionUniformLocations.halfFOVInRadians === -1 ||
          this.reprojectionUniformLocations.eyeLeft === null ||
          this.reprojectionUniformLocations.eyeLeft === -1 ||
          this.reprojectionUniformLocations.eyeRight === null ||
          this.reprojectionUniformLocations.eyeRight === -1
        ) {
          console.error(
            "!!! Failed to get all required uniform locations for reprojection shader.",
          );
        ...
        console.warn(
          `Reprojection shader setup failed or skipped: ${
            (error as Error).message
          }. Reprojection will be disabled.`,
        );
  ...
  renderPanoramaFromData(
    ...
    const useReprojection = this.reprojectionEnabled && !!this.reprojectionShaderProgram &&
      !!currentPoseMatrix;

    // Select the shader program and uniform locations based on the decision
    const activeShaderProgram = useReprojection
      ? this.reprojectionShaderProgram
      : this.shaderProgram;
    const activeUniforms = useReprojection
      ? this.reprojectionUniformLocations
      : this.uniformLocations;

    // --- 1. Check Resources ---
    if (
      !this.fbo || !activeShaderProgram || !this.vao ||
      !this.leftEyeTexture || !this.rightEyeTexture || !this.outputTexture ||
      !this.leftEyePbo || !this.rightEyePbo
    ) { // Added PBO check
      console.error(
        `Render call failed (${
          useReprojection ? "Reprojection" : "Standard"
        }): Essential OpenGL resources not initialized (including PBOs).`,
      );
    ...
    if (
      this.pboEyeWidth !== eyeWidth || this.pboEyeHeight !== eyeHeight ||
      ...
      gl.BufferData(
        ...
        Deno.UnsafePointer.create(BigInt(this.pboEyeBufferSize)),
        null,
        gl.STREAM_DRAW,
      );
    ...
    if (useReprojection) {
      ...
      uniformsValid = reprojUniforms.renderPoseMatrix !== null &&
        reprojUniforms.renderPoseMatrix !== -1 &&
      ...
      uniformsValid = stdUniforms.lookRotation !== null && stdUniforms.lookRotation !== -1 && // Use lookRotation here
    ...
    if (!uniformsValid) {
      ...
      console.error(
        `Render call failed: Required uniform locations not found for ${shaderMode} shader.`,
      );
    ...
    const leftPboPtr = gl.MapBufferRange(
      gl.PIXEL_UNPACK_BUFFER,
      null,
      Deno.UnsafePointer.create(BigInt(this.pboEyeBufferSize)),
    ...
    const leftPboView = new Uint8Array(
      Deno.UnsafePointerView.getArrayBuffer(
        leftPboPtr as Deno.PointerObject<unknown>,
        this.pboEyeBufferSize,
      ),
    );
    ...
    const rightPboPtr = gl.MapBufferRange(
      gl.PIXEL_UNPACK_BUFFER,
      null,
      Deno.UnsafePointer.create(BigInt(this.pboEyeBufferSize)),
      gl.MAP_WRITE_BIT | gl.MAP_INVALIDATE_BUFFER_BIT,
    ...
    const rightPboView = new Uint8Array(
      Deno.UnsafePointerView.getArrayBuffer(
        rightPboPtr as Deno.PointerObject<unknown>,
        this.pboEyeBufferSize,
      ),
    );
    ...
    if (!this.checkGLError(`use program (${useReprojection ? "Reprojection" : "Standard"})`)) {
      return;
    }
  ...
  createTextureFromData(pixels: Uint8Array, width: number, height: number, noFlip?: boolean): void {
    let pixelsX = pixels;
    if (!noFlip) {
      if (!this.textureUploadBuffer || this.textureUploadBuffer.length !== pixels.length) {
        this.textureUploadBuffer = new Uint8Array(pixels.length);
      }
      pixelsX = flipVertical(pixels, width, height, this.textureUploadBuffer);
    }
    ...
    if (width !== this.textureUploadWidth || height !== this.textureUploadHeight) {
      ...
      this.textureUploadWidth = width;
      this.textureUploadHeight = height;
    } else {
      gl.TexSubImage2D(
        gl.TEXTURE_2D,
        0,
  ...
  createTextureFromBgraScreenshot(pixels: Uint8Array, width: number, height: number): void {
    ...
    this.checkGLError("upload BGRA screenshot texture data");
  }
